...
interface AwardPageClientProps {
  awardId: string;
}

async function fetchAwardDetail(id: string) {
  const response = await fetch(`/api/polyrouter/awards/${id}`);
  if (!response.ok) {
...
export default function AwardPageClient({ awardId }: AwardPageClientProps) {
  ...
  const { data: response, isLoading, error } = useQuery({
    ...
    queryFn: () => fetchAwardDetail(awardId),
    staleTime: 30000,
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-2 text-gray-600">Загрузка данных награды...</p>
        </div>
      </div>
    );
  }

  if (error || !response?.awards || response.awards.length === 0) {
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          <p className="text-red-600 mb-4">Ошибка загрузки награды</p>
          <button
            onClick={() => router.push("/")}
  ...
  const award = response.awards[0];
  
  // Преобразуем award в формат Event для совместимости
  const markets = award.markets?.flatMap((m: any) => 
    m.candidates?.map((candidate: any) => ({
      id: `${award.id}-${m.platform}-${candidate.player_name}`,
      platform: m.platform as Platform,
      originalTitle: `${award.award_name} - ${candidate.player_name}`,
      probability: candidate.odds?.implied_probability ? candidate.odds.implied_probability * 100 : 0,
      volume: candidate.volume_24h,
      link: `#`,
      updatedAt: new Date().toISOString(),
      eventId: award.id,
      eventName: award.award_name,
      category: award.league || 'Sports',
    })) || []
  ) || [];

  const event = {
    id: award.id,
    normalizedTitle: award.award_name,
    description: award.metadata?.description,
    category: award.league || 'Sports',
    tags: [],
    markets,
  };
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="bg-white rounded-lg shadow p-6 mb-6">
          ...
          {award.metadata?.description && (
            <p className="text-gray-600 mb-4">{award.metadata.description}</p>
          )}
          <div className="flex flex-wrap gap-4 text-sm">
            <div>
              <span className="text-gray-500 mr-2">Лига:</span>
              <span className="font-medium">{award.league}</span>
            </div>
            <div>
              <span className="text-gray-500 mr-2">Сезон:</span>
              <span className="font-medium">{award.season}</span>
            </div>
            <div>
